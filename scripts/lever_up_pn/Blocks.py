...
class RecurrentRelationalModel_many(Layer):
    def __init__(self, input_size,n_of_features,filters,relnetpart1_model=None,relnetpart2_model=None,reuse_model=False,timestep_diff=5,lookstart=50,**kwargs):
        ...
        x=input2
        x = Dense(filters[-1],activation='linear')(x)
...
#        self.trainable_weights = self.relnetpart1.trainable_weights,self.relnetpart2.trainable_weights
#         self.relnetpart1.build((None,self.n_of_features)) #,self.input_size
#         self.trainable_weights = self.relnet.trainable_weights
#         self.relnetpart2.build((None,self.n_of_features)) #,self.input_size
#         self.trainable_weights = self.relnet.trainable_weights
    ...
    def call(self, X ):              
        ...
        #print(output[:,np.random.randint(self.timestep_diff),:])
...
class RecurrentRelationalModel2(Layer):
    def __init__(self, input_size,n_of_features,filters,rm=None,reuse_model=False,timestep_diff=5,**kwargs):
        self.input_size=input_size
        self.n_of_features=n_of_features
        self.timestep_diff=timestep_diff
        ...
        super(RecurrentRelationalModel2, self).__init__(**kwargs)
    ...
    def call(self, X):              
        x = K.reshape(X[0],(-1,self.timestep_diff,self.n_of_features))
        x1 =K.reshape(X[1],(-1,4))

        output = self.relnet.call([x,x1])
        output= K.reshape(output,(-1,self.input_size,self.output_size))
        return output
